...
int getWinner(int board[9])
{
	...
	for (int x = 0; x < 3; x++)
	{
		...
		{ // Checks diagonals. Doesn't check if x == 2.
			winner = board[4];
			break;
		}
	}
...
int exceptionalCase(int board[9])
{
	...
	for (int caseIndex = 0; caseIndex < 2; caseIndex++)
	{
		for (int rotation = 0; rotation < 4; rotation++)
		{
			...
			for (int x = 0; x < 9; x++)
			{
				if (newBoard[x] == cases[caseIndex][x])
					match++;
				else
					break;
			}
			if (match == 9)
				return answers[caseIndex][rotation];
		}
	}
	return -1;
}
...
int main()
{
	...
	while (true)
	{
		...
		if (good)
			...
			board[4] = 2; // Middle.
		else if (exceptionalCase(board) > -1)
			board[exceptionalCase(board)] = 2; // Exception boards.
		else if (getSpace(board, corners) != -1)
			board[getSpace(board, corners)] = 2; // Corners
		else
			board[getSpace(board, sides)] = 2; // Sides
